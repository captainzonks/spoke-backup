...
# Core sources always backed up. Add extra paths via BACKUP_EXTRA_SOURCES
# (space-separated) — mount them into the container first via override.
# modules.yml is a single file; copy it into staging so /staging carries it.
readonly _CORE_SOURCES=(
    /sources/secrets
    /sources/shared-env
    /sources/appdata
    /staging
)
IFS=' ' read -ra _EXTRA_SOURCES <<< "${BACKUP_EXTRA_SOURCES:-}"
readonly SOURCES=("${_CORE_SOURCES[@]}" "${_EXTRA_SOURCES[@]}")
