...
#              Clusters are defined by BACKUP_PG_CLUSTERS (space-separated
#              name:host pairs). Output written to
...
# Required env:
#   BACKUP_STAGING_DIR
#   BACKUP_PG_CLUSTERS  space-separated name:host pairs, e.g. "hub:postgres-hub"
# Required secrets (one per cluster, named by convention):
#   /run/secrets/postgres_<name>_backup_password
...
dump_cluster() {
    ...
    local host=$2
    local secret_file="/run/secrets/postgres_${cluster_name}_backup_password"
    local staging
...
main() {
    require_env BACKUP_STAGING_DIR BACKUP_PG_CLUSTERS
    ...
    local -a clusters
    # IFS=' ' is command-scoped (only applies to this `read`) so we can split
    # on spaces despite the script-level IFS=$'\n\t'. Without the prefix,
    # `read -ra` would honor the script IFS and put the entire string into
    # one array element.
    ...
    for pair in "${clusters[@]}"; do
        name="${pair%%:*}"
        host="${pair##*:}"
        dump_cluster "$name" "$host" || failures=$((failures + 1))
    done
